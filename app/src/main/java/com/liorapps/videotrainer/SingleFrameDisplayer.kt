...
class SingleFrameDisplayer(
    ...
    */
    suspend fun seekToFrame(targetIndex: Int) {
        ...
        require(keyframeIndex >= 0) { "Frame $targetIndex is not displayable" }
        Timber.d("seekToFrame() targetIndex=$targetIndex  keyframeIndex=$keyframeIndex")
        ...
        Timber.d("#######S <<<LOOP>>> targetIndex=$targetIndex lastFrameIndex=$lastFrameIndex safetyLimit=$safetyLimit")
        while (!rendered  &&  safetyLimit-- > 0) {
            ...
            if (nextToFeed <= lastToFeed) {
                ...
                if (inputIndex >= 0) {
                    ...
                    if (nextToFeed == lastToFeed) flags = flags or MediaCodec.BUFFER_FLAG_END_OF_STREAM
...
//    class VideoFrameNavigator(private val decoder: SingleFrameDisplayer) {
    ...
    suspend fun displayFrameByRelativeLocation(percentage: Float) {
        ...
        val target = first + (percentage.coerceIn(0f, 1f) * (displayableCount - 1)).roundToInt()
        seekToFrame(target)
    }
...
//    }
    ...
    suspend fun setupDecoder(surface: Surface): Boolean {
